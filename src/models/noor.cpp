...
// (qwen3next/kimi-linear/qwen3.5 all have head_dim_k==head_dim_v). Rather than touch that
...
void llama_model_noor::load_arch_tensors(llama_model_loader &) {
    ...
    // 24 square-state heads by the converter; see the hparam-loading comment
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (!hparams.is_recr(i)) {
            ...
            // alpha/beta projections, all pre-repacked to n_head_gdn=24 /
            // head_dim_gdn=128 by the converter.
            ...
            // gated-RMSNorm weight, pre-tiled [head_dim_gdn, n_head_gdn] by the
            // converter (eps=1e-5, applied via a direct ggml_rms_norm call in the
            // graph builder -- NOT build_norm's shared hparams.f_norm_rms_eps=1e-6).
...
// Noor's three convs are uniformly d_inner=n_head_gdn*head_dim_gdn wide, so this is called
// identically for qkv_idx 0/1/2 with the SAME d_inner/head_dim/n_head every time.
