...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_TQ3_S] = {
        .type_name                = "tq3_s",
        .blck_size                = QK_TQ3_S,
        .type_size                = sizeof(block_tq3_s),
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_tq3_s,
        .from_float_ref           = (ggml_from_float_t) quantize_row_tq3_s_ref,
    },
...
enum ggml_type ggml_ftype_to_ggml_type(enum ggml_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case GGML_FTYPE_MOSTLY_TQ3_0:         wtype = GGML_TYPE_TQ3_0;    break;
        case GGML_FTYPE_MOSTLY_TQ4_0:         wtype = GGML_TYPE_TQ4_0;    break;
...
struct ggml_tensor * ggml_gated_delta_net(
    ...
    GGML_ASSERT(ggml_nelements(state) == S_k * S_v * H * n_seqs);

    // concat output and new_state into a single tensor
    // output: S_v * H * n_tokens * n_seqs, state: S_k * S_v * H * n_seqs
    const int64_t ne[4] = { S_v * H, n_tokens * n_seqs + S_k * n_seqs, 1, 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
...
size_t ggml_quantize_chunk(
    ...
    switch (type) {
        ...
        case GGML_TYPE_TQ3_S:   result = quantize_tq3_s(src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
